...
figure
subplot(1, 2, 2);
title('Przesuniêta maska');
imshow(warpedImage);
subplot(1, 2, 1);
imshow(co);
title('Oryginalna maska');
%% obwiednia
mask = uint8(warpedImage);
im_mask = mask.*im_cropped;
figure(6455)
imshow(im_mask)
im_mask2= im_mask(im_mask~=0);

% pd = fitdist(double(im_mask2),'nakagami');
% 
% figure
% xgrid = linspace(1,1,255)';
% h=histogram(double(im_mask2),'Normalization','pdf');
% h.NumBins =20;
% h.BinEdges = [1:255];
% pdfEst = pdf(pd,xgrid);
% hold on
